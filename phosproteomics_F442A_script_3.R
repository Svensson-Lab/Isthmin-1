...
dev.copy2pdf(file=paste0("plots/heatmap_top100_grid_no_na",paste0(".pdf")), 
             width=6.5, height=12)
dev.off()
h
dev.copy2pdf(file=paste0("plots/heatmap_top100_nogrid_no_na",paste0(".pdf")), 
             width=6.5, height=12)
dev.off()
...
sig.tab[grep("neuro",sig.tab$Term),] #in 27 terms
sig.tab[grep("immune",sig.tab$Term),] #in 14 terms 
sig.tab[grep("adipo",sig.tab$Term),] #in 0 terms
sig.tab[grep("fat cell",sig.tab$Term),] #in 4 terms
sig.tab[grep("liver|hepat",sig.tab$Term),] #liver in 1 term, hepatin 1 term
sig.tab[grep("pancrea",sig.tab$Term),] #in 0 terms 
sig.tab[grep("muscle",sig.tab$Term),] #in 36 terms 
sig.tab[grep("lung|pulmo",sig.tab$Term),] #lung in 2 terms, pulmo in 0 terms
sig.tab[grep("kidney",sig.tab$Term),] #in 2 terms
sig.tab[grep("spleen",sig.tab$Term),] #in 0 terms
sig.tab[grep("intest",sig.tab$Term),] #in 0 terms
sig.tab[grep("adren",sig.tab$Term),] #in 0 terms
sig.tab[grep("skin",sig.tab$Term),] #in 2 terms
sig.tab[grep("bladder",sig.tab$Term),] #in 0 terms
sig.tab[grep("sex",sig.tab$Term),] #in 4 terms
sig.tab[grep("thyroid",sig.tab$Term),] #in 4 terms
sig.tab[grep("thymus",sig.tab$Term),] #in 4 terms
...
write.table(sig.tab,file=paste0("tables/",
  paste0("2022-03-29_GOs_ISMvsBSA",".tsv")),sep="\t")
dim(sig.tab)
...
simMatrix <- calculateSimMatrix(sig.tab$GO.ID,
                                ...
                                ont="BP",
                                method="Rel")
saveRDS(simMatrix,"GOsimilaritymatrix.rds")
...
write.table(goplot,row.names=FALSE,file=paste0("tables/",
                                paste0("GOs_ISMvsBSA_clust",".tsv")),sep="\t")
...
p1
dev.copy2pdf(file=paste0("plots/pathways_PCOA_plot",paste0(".pdf")), width=5.8, height=4)
dev.off()
p1
ggsave(file=paste0("plots/",paste0("pathways_PCOA_plot",".svg")),
       width=5.8, height=4)
